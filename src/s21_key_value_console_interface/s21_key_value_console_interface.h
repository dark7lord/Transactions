...
class KeyValueConsoleInterface {
 ...
 private:
  ...
  KeyValueConsoleInterface& operator=(KeyValueConsoleInterface&& other) =
      delete;
  ~KeyValueConsoleInterface() = delete;

  static void tokenize_(const std::string& user_input,
                        std::vector<std::string>& out) noexcept;
  static bool validateTokens_(const std::vector<std::string>&,
                              std::size_t) noexcept;
  static Value parse_value_dash_(const std::string& first_name,
