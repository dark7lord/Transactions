...
static const std::string MENU_MESSAGE =
    ...
    "	UPLOAD <filename>: load the storage from a .dat file_for_test.dat.\n"
    "	SHOWALL: get all records that are in the key-value storage at the "
    "moment.\n"
    ...
    "	DEL <key>: delete the key and the corresponding value (returns true "
    "on\n"
    "		success).\n"
    "	EXPORT <filename>: save the storage as a .dat file_for_test.dat.\n"
    "	UPDATE <key> <last name> <first name> <year of birth> <city>\n"
    "		<number of current coins>:\n"
    "		updates the value by the corresponding key if such a key "
    "exists. If\n"
    "		there is a field that is not planned to change, it is replaced "
    "by a\n"
    "		dash '-'.\n"
    ...
    "		<number of current coins>:\n"
    "		restore the key (or keys) according to a given value. f any "
    "fields will\n"
...
void KeyValueConsoleInterface::start() noexcept {
  ...
  runArrows();
  while (!std::cin.eof() && !KeyValueConsoleInterface::storage_ &&
         !KeyValueConsoleInterface::on_exit_) {
    startMenu();
  }
...
void KeyValueConsoleInterface::menu_(
...
void KeyValueConsoleInterface::files_(
    const std::vector<std::string>& tokens) noexcept {
  if (validateTokens_(tokens, 1)) {
    return;
  }
  std::cout << GREY << "Next files are available in current directory:" << NONE
            << std::endl;
...
void runStartUserInput(const std::string& user_input) noexcept {
  ...
  std::cout << RED << "> ERROR: command " << tokens[0] << " is not defined"
            << NONE << std::endl;
}
...
static void runArrows() noexcept {
  for (int i = 0; i < 60; ++i) {
    ...
    for (int j = 0; j < i; ++j) write(0, " ", 1);
    write(0, "> > > >", 7);
    for (int j = 0; j < 60 - i; ++j) write(0, " ", 1);
    usleep(10000);
    write(0, "\r", 1);
  }
  for (int j = 0; j < 67; ++j) write(0, " ", 1);
  write(0, "\r", 1);
  std::cout << NONE << std::flush;
}
