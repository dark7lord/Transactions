...
*/
std::list<HashTable::Node>::iterator HashTable::first_bigger_or_equal_hash_(
    const HashCode& hash, std::list<HashTable::Node>& lst) noexcept {
  auto last = lst.end();
...
bool HashTable::check_key_exists_(
    const Key& key, const HashCode& hash, std::list<HashTable::Node>& lst,
