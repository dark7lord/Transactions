...
TEST(HashTableUpdate, UpdatePart) {
	...
	EXPECT_EQ(*(table.get("hello, world")), s21::Value({ "Hel", "lo", "0", "world", "2" }));
}
...
TEST(HashTableRename, ThrowOnNotExists) {
	s21::HashTable table;
	EXPECT_THROW(table.rename("1", "2"), s21::IKeyValueStorage::KeyNotExistsException);
}
...
TEST(HashTableUpload, GoodFiles) {
	s21::HashTable table;
	ASSERT_NO_THROW(table.upload("tests/test_files/file_for_test.dat"));
	EXPECT_EQ(table.keys().size(), 1);
}

TEST(HashTableUpload, BadFiles) {
	s21::HashTable table;
	EXPECT_THROW(table.upload("tests/test_files/bad_file_for_test1.dat"), s21::IKeyValueStorage::CantOpenFile);
	EXPECT_THROW(table.upload("tests/test_files/bad_file_for_test2.dat"), s21::IKeyValueStorage::CantOpenFile);
}
